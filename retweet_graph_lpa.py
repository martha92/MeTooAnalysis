...
# Read retweet graph
df = pd.read_csv('retweetgraph.txt', sep='\t', header=None, names=['src', 'dst', 'screen_name', 'retweet_user', 'count'])
sdf = spark.createDataFrame(df)

# Define nodes and edges
nodes = sdf.select('src', 'screen_name').union(sdf.select('dst', 'retweet_user')).select(F.col('src').alias('id'), F.col('screen_name').alias('name')).drop_duplicates()
edges = sdf.select('src', 'dst', 'count').withColumnRenamed('count', 'weight') 
...
nodes.select('id', F.col('name').alias('label')).coalesce(1).write.csv('nodes', sep=',', header=True, mode='overwrite')
